    ...
    public void insertarDatos (Pelicula... peliculas) {
        ...
        try (Connection con = DriverManager.getConnection (CONNECTION_STRING);
            ...
            for (Pelicula p : peliculas) {
                if (1 == stmt.executeUpdate (String.format (sql, p.getId ().toString (), p.getNombre (),
                        p.getValoracion (), p.getDirector (), p.getDuracion (), p.getEdad (), Genero.ACCION))) {
    ...
    public List <Pelicula> obtenerDatos () {
        ...
        try (Connection con = DriverManager.getConnection (CONNECTION_STRING);
            ...
            while (rs.next ()) {
                ...
                // pelicula.setGeneros(new ArrayList<Genero>(Arrays.asList((new
                // Genero[])rs.getArray("GENEROS"))));
