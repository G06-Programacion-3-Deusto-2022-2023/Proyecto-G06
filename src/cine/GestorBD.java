...
public class GestorBD {
	...
	public void insertarDatos(Pelicula... peliculas ) {
		...
		try (Connection con = DriverManager.getConnection(CONNECTION_STRING);
			...
			String sql = "INSERT INTO PELICULAS (ID ,NOMBRE, VALORACION, DIRECTOR, DURACION, EDAD_RECOMENDADA, GENEROS) VALUES ('%s','%s', '%f', '%s', '%s', '%s', '%s');";
			
			System.out.println("- Insertando peliculas...");
			
			//Se recorren los clientes y se insertan uno a uno
			for (Pelicula p : peliculas) {
				if (1 == stmt.executeUpdate(String.format(sql, p.getId().toString(), p.getNombre(), p.getValoracion(), p.getDirector(), p.getDuracion(), p.getEdad(),Genero.ACCION))) {					
	...
	public List<Pelicula> obtenerDatos() {
		...
		try (Connection con = DriverManager.getConnection(CONNECTION_STRING);
			...
			while (rs.next()) {
				...
				//pelicula.setGeneros(new ArrayList<Genero>(Arrays.asList((new Genero[])rs.getArray("GENEROS"))));
	...
	public void actualizarValoracion(Pelicula pelicula, double newValoracion) {
		...
		try (Connection con = DriverManager.getConnection(CONNECTION_STRING);
			...
			String sql = "UPDATE PELICULA SET VALORACION = '%s' WHERE ID = '%s';";
			
			int result = stmt.executeUpdate(String.format(sql, newValoracion, pelicula.getId().toString()));
